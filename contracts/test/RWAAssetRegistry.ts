...
describe("RWAAssetRegistry", function () {
  ...
  async function deployFixture() {
    const [admin, factory, compliance, payment] =
    ...
    await registry.grantRole(
      await registry.ASSET_FACTORY_ROLE(),
      factory.address
    );
    await registry.grantRole(
      await registry.COMPLIANCE_ROLE(),
      compliance.address
    );
    await registry.grantRole(
      await registry.PAYMENT_COLLECTOR_ROLE(),
      payment.address
    );
  ...
  describe("Activation", function () {
    it("activates asset after compliance approval", async function () {
      const { registry, factory, compliance } =
      ...
      await expect(
        registry.connect(compliance).activateAsset(assetId)
      ).not.to.be.reverted;

      const asset = await registry.assets(assetId);
      expect(asset.currentStatus).to.equal(1); // ACTIVE
    });
  });

  describe("Default detection", function () {
    it("marks asset as DEFAULTED after missed payments", async function () {
      const { registry, factory, payment, compliance } =
        await loadFixture(deployFixture);
      ...
      const defaultThresholdDays = await registry.defaultThresholdDays();

      await time.increase((Number(defaultThresholdDays) + 1) * 24 * 60 * 60);
